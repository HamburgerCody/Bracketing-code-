...
def start_gui():
    ...
    select_button = tk.Button(app, text="Select Document or Folder", font=("Arial", 14), command=select_item)
...
# Function to select an item (either folder or file)
def select_item():
    global selected_item
    selected_item = filedialog.askopenfilename(initialdir=initial_working_directory)
    if selected_item:
        process_selected()  # Automatically process the selected item
