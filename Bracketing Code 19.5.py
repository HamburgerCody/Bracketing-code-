...
# Output folder for processed files (without creating a subfolder with the project name)
...
def process_word_document(input_filename, word_list, result_folder, num_iterations=3):
    try:
        ...
        output_filename = re.sub(r'[\/:*?"<>|]', '_', os.path.basename(input_filename))
        full_output_path = os.path.join(result_folder, output_filename + "_Processed.docx")
