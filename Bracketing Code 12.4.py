...
# Function to process all HTML files within a folder and its subfolders
def process_folder(folder_path):
    try:
        ...
        # Search for HTML files within the folder and its subfolders
        html_files = glob.glob(os.path.join(folder_path, '**/*.html'), recursive=True)

        # Process each HTML file and create a corresponding Word document
        for html_file in html_files:
            # Determine the relative path within the folder
            relative_path = os.path.relpath(html_file, folder_path)
            ...
            with open(html_file, 'r', encoding='utf-8') as html_file_content:
...
process_button = tk.Button(app, text="Process Word Document", font=("Arial", 14), command=process_document)
process_button.pack(pady=10)

# Button to process a folder
process_folder_button = tk.Button(app, text="Process Folder", font=("Arial", 14), command=process_folder_dialog)
