...
output_folder_documents = os.path.join(initial_working_directory, "Processed", "Processed Documents")
output_folder_folders = os.path.join(initial_working_directory, "Processed", "Processed Folders")
...
def process_document(doc_path):
    try:
        ...
        if processing_enabled:  # Check if processing is enabled
            ...
            output_filename = os.path.join(output_folder_documents, output_filename + ".docx")
...
def process_file(file_path):
    try:
        ...
        if processing_enabled:  # Check if processing is enabled
            ...
            output_filename = os.path.join(output_folder_documents, output_filename + ".txt")
...
def process_folder(folder_path):
    try:
        ...
        processed_folder = os.path.join(output_folder_folders, os.path.basename(folder_path))
        ...
        for md_file in md_files:
            process_markdown_file(md_file, saved_word_list, processed_folder)
...
def process_markdown_file(md_file, word_list, output_folder):
    try:
        ...
        output_folder_path = os.path.join(output_folder, os.path.splitext(sanitized_relative_path)[0])
...
def start_gui():
    ...
    select_doc_button = tk.Button(button_frame, text="Select Document", font=("Arial", 14), command=select_document, state=tk.DISABLED)
    select_doc_button.grid(row=0, column=0, padx=5)

    select_folder_button = tk.Button(button_frame, text="Select Folder", font=("Arial", 14), command=select_folder, state=tk.DISABLED)
