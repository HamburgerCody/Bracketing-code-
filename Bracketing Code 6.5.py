...
import re  # Added re module for sentence splitting
...
def add_double_brackets_to_words_in_text(input_text, word_list):
    ...
    for word in input_text.split():
        # Check if the word (stripped of leading/trailing spaces) matches any word in the list (case-insensitive)
        if any(word.strip().lower() == item.lower() for item in word_list):
...
def process_word_document(input_filename, result_filename, word_list):
    try:
        ...
        # Process the entire document as a single text block
        full_text = ' '.join(paragraph.text.strip() for paragraph in doc.paragraphs)
        processed_text = add_double_brackets_to_words_in_text(full_text, word_list)
