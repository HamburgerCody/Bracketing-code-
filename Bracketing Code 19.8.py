...
def process_word_document(input_filename, word_list, result_folder, num_iterations=3):
    try:
        ...
        output_filename = re.sub(r'[\/:*?"<>|]', '_', os.path.splitext(os.path.basename(input_filename))[0] + "_Processed.docx")
...
def process_document(doc_path):
    try:
        ...
        if processing_enabled:  # Check if processing is enabled
            ...
            if process_word_document(doc_path, saved_word_list, output_documents_folder):
                messagebox.showinfo("Success", f"Processing completed successfully. Result saved as '{os.path.basename(doc_path)}'")
            else:
...
def process_file(file_path):
    try:
        ...
        if processing_enabled:  # Check if processing is enabled
            ...
            # Create a sanitized output file name by replacing problematic characters
            output_filename = re.sub(r'[\/:*?"<>|]', '_', os.path.splitext(os.path.basename(file_path))[0] + "_Processed.txt")
            output_filename = os.path.join(output_documents_folder, output_filename)
            ...
            messagebox.showinfo("Success", f"Processing completed successfully. Result saved as '{os.path.basename(output_filename)}'")
    except Exception as e:
...
def process_markdown_file(md_file, word_list):
    try:
        ...
        sanitized_relative_path = re.sub(r'[\/:*?"<>|]', '_', os.path.splitext(relative_path)[0])
        output_folder_path = os.path.join(output_folders_folder, sanitized_relative_path)

        # Create the output folder if it doesn't exist
        os.makedirs(output_folder_path, exist_ok=True)

        # Create the corresponding output file path in the processed folder with a .docx extension
        output_file_name = os.path.splitext(os.path.basename(md_file))[0] + "_Processed.docx"
