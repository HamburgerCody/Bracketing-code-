...
# Function to process a Word document with modified file naming
def process_word_document(input_filename, word_list, result_folder, num_iterations=3):
    try:
        ...
        # Create a sanitized output file name by appending "_Processed" to the original file name
        original_filename = os.path.basename(input_filename)
        filename, file_extension = os.path.splitext(original_filename)
        output_filename = f"{filename}_Processed{file_extension}"
        ...
        # Save the result with the modified file name
...
def process_folder(folder_path):
    try:
        ...
        processed_folder = os.path.join(output_folders_folder, "Processed_Documents")
