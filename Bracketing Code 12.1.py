...
import shutil  # Added for folder duplication
...
# Function to process HTML files in a folder and save the processed content into Word documents
def process_html_files_in_folder(input_folder, output_folder):
    ...
    for html_file in html_files:
        ...
        # Process the HTML content (you can use your existing processing function here)
        processed_content = process_html_content(html_content)

        # Create a corresponding output Word document file
        output_filename = os.path.basename(html_file).replace(".html", "_processed.docx")
        output_path = os.path.join(output_folder, output_filename)

        # Save the processed content into a Word document
        save_text_as_word_document(processed_content, output_path)

def process_html_content(html_content):
    # You can implement your HTML processing logic here
    # For example, you can use BeautifulSoup to parse and manipulate the HTML content
    # Return the processed content as a string
    processed_html = html_content  # Placeholder, replace with your actual processing logic
    return processed_html

def save_text_as_word_document(text, output_path):
...
# Function to duplicate a folder
def duplicate_folder(input_folder, output_folder):
    try:
        shutil.copytree(input_folder, output_folder)
        return True
    except Exception as e:
        logging.error(f"Error: Unable to duplicate folder. Error: {str(e)}")
        return False

# Function to duplicate a folder, process HTML files in the duplicated folder, and save as Word documents
def duplicate_and_process_folder(input_folder):
    try:
        # Create a duplicate folder
        duplicated_folder = input_folder + "_duplicated"
        if not duplicate_folder(input_folder, duplicated_folder):
            return False

        # Process HTML files in the duplicated folder
        process_html_files_in_folder(duplicated_folder, duplicated_folder)

        return True
    except Exception as e:
        logging.error(f"Error: An error occurred while duplicating and processing the folder. Error: {str(e)}")
        return False
...
# Add the "Duplicate and Process Folder" button
duplicate_and_process_button = tk.Button(button_frame, text="Duplicate and Process Folder", font=("Arial", 14), command=duplicate_and_process_folder_wrapper)
duplicate_and_process_button.grid(row=0, column=3, padx=5)
