...
# Variable to store the selected Word document or folder
selected_document = None

# Function to select a folder or a Word document
def select_document_or_folder(text_box):
    global selected_document
    selected_document = None  # Reset the selected document
    file_path = filedialog.askopenfilename(filetypes=[("Word Documents", "*.docx")], initialdir=initial_working_directory)
    if file_path:
        selected_document = file_path  # Store the selected document path
        # Display the selected document path in the text box
        text_box.delete("1.0", tk.END)
        text_box.insert(tk.END, selected_document)
...
# Function to process the selected Word document or folder
def process_selected_document(text_box):
    global selected_document
    try:
        if selected_document:
            if os.path.isdir(selected_document):
                # Selected a folder, process the folder
                process_folder(selected_document)
            elif os.path.isfile(selected_document):
                # Selected a document, process the document
                output_filename = re.sub(r'[\/:*?"<>|]', '_', selected_document)
                output_filename = os.path.join(initial_working_directory, "Processed_Documents", output_filename + ".docx")

                # Process the Word document and display a message based on success or failure
                if process_word_document(selected_document, saved_word_list, output_filename):
        ...
        logging.error(f"Error: An error occurred while processing the document or folder. Error: {str(e)}")
...
def process_folder(folder_path):
    try:
        ...
        for md_file in md_files:
            ...
            output_folder_path = os.path.join(processed_folder, os.path.splitext(sanitized_relative_path)[0])
            ...
            # For now, we will just add double brackets to words as in the Word document processing
            processed_md_content = add_double_brackets_to_words_in_text(md_content, saved_word_list)
...
def start_gui():
    ...
    # Button to select a Word document or folder
    select_document_or_folder_button = tk.Button(app, text="Select Document or Folder", font=("Arial", 14), command=lambda: select_document_or_folder(text_box))
    select_document_or_folder_button.pack(pady=10)

    # Button to process the selected Word document or folder
    process_document_button = tk.Button(app, text="Process Document or Folder", font=("Arial", 14), command=lambda: process_selected_document(text_box))
    process_document_button.pack(pady=10)
