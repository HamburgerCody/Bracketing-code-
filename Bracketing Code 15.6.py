...
# Variable to store the selected Word document
...
def process_selected_document(text_box):
    global selected_document
    try:
        if selected_document and os.path.isfile(selected_document):
...
def start_gui():
    ...
    # Button to process the selected Word document
    process_document_button = tk.Button(app, text="Process Document", font=("Arial", 14), command=lambda: process_selected_document(text_box))
