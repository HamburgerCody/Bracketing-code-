...
# Initialize variables to store the selected folder and document paths
...
def process_selected_document():
    ...
    if selected_document:
        process_document(selected_document)
...
def process_folder_dialog():
    global selected_folder
    selected_folder = filedialog.askdirectory(initialdir=initial_working_directory)
    if selected_folder:
        process_folder(selected_folder)

# Function to open a Word document selection dialog and start processing the selected document
def process_document_dialog():
    global selected_document
    selected_document = filedialog.askopenfilename(filetypes=[("Word Document", "*.docx")], initialdir=initial_working_directory)
    if selected_document:
        process_document(selected_document)

# Function to start processing based on the selected folder or document
def start_processing():
    global selected_folder, selected_document
    if selected_folder:
        process_folder(selected_folder)
...
# Button to add a Word document for processing
add_word_doc_button = tk.Button(app, text="Add Word Document", font=("Arial", 14), command=process_document_dialog)
add_word_doc_button.pack(pady=10)
