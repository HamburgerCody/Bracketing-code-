...
if __name__ == "__main__":
    while True:
        ...
        if choice == "1":
            # Prompt the user to enter the word list in the specified format
            print("Enter the word list (each word enclosed in double quotes and separated by newlines):")
            word_list_text = ""
            while True:
                line = input()
                if line == "":
            ...
            last_used_word_list = [line.strip('"') for line in word_list_text.split('\n') if line.strip('"')]
