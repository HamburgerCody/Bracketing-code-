...
def process_word_document(input_filename, word_list, result_filename):
    try:
        ...
        messagebox.showinfo("Success", f"Processing completed successfully. Result saved as '{result_filename}'")
    except Exception as e:
...
update_word_list_button = tk.Button(app, text="Update Word List", font=("Arial", 14), command=update_word_list)
process_button = tk.Button(app, text="Process", font=("Arial", 14), command=process_document)
add_word_button = tk.Button(app, text="Add Word", font=("Arial", 14), command=add_word)
remove_word_button = tk.Button(app, text="Remove Word", font=("Arial", 14), command=remove_word)
...
def update_word_list():
    ...
    saved_word_list = text_box.get("1.0", tk.END).splitlines()
