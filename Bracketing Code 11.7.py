...
def limit_brackets(word_list):
    for i in range(len(word_list)):
        word = word_list[i]
        # Use regular expressions to remove extra brackets while preserving double brackets
        word = re.sub(r'\[+', '[', word)  # Replace consecutive '[' with a single '['
        word = re.sub(r'\]+', ']', word)  # Replace consecutive ']' with a single ']'
        word = re.sub(r'\[\[([^[\]]*?)\]\]', r'[[\1]]', word)  # Preserve double brackets
        word_list[i] = word
