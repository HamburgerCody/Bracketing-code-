...
def select_document():
    document_path = filedialog.askopenfilename(filetypes=[("Word Documents", "*.docx")], initialdir=initial_working_directory)
    if document_path:
...
def process_word_document(input_filename, word_list, result_filename, num_iterations=3):
    try:
        ...
        output_folder_path = os.path.join(initial_working_directory, "Processed_Documents")
...
def process_document(selected_document):
    try:
        if selected_document:
            # Create a sanitized output file name by replacing problematic characters
            output_filename = re.sub(r'[\/:*?"<>|]', '_', os.path.basename(selected_document))
            output_filename = os.path.join(initial_working_directory, "Processed_Documents", output_filename)
...
def start_gui():
    ...
    select_document_button = tk.Button(app, text="Select Document", font=("Arial", 14), command=select_document)
    ...
    process_button = tk.Button(app, text="Process", font=("Arial", 14), command=lambda: process_selected(text_box))
...
def process_selected(text_box):
    selected_item = text_box.get("1.0", "end-1c")
    if os.path.isfile(selected_item):
        process_document(selected_item)
    elif os.path.isdir(selected_item):
        process_folder(selected_item)
