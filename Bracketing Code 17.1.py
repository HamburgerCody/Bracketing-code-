...
output_folder = initial_working_directory  # You can change this to your desired output folder
...
# Function to select a file and double bracket its content
def select_file():
    file_path = filedialog.askopenfilename(initialdir=initial_working_directory)
    if file_path:
...
def process_markdown_file(md_file, word_list):
    try:
        ...
        output_folder_path = os.path.join(output_folder, "Processed_Documents", os.path.splitext(sanitized_relative_path)[0])
...
def start_gui():
    ...
    # Buttons to select a Word document, file, or folder
    ...
    select_file_button = tk.Button(button_frame, text="Select File", font=("Arial", 14), command=select_file)
    ...
    update_word_list_button.grid(row=1, column=0, padx=5)

    save_word_list_button = tk.Button(button_frame, text="Save Word List", font=("Arial", 14), command=lambda: save_word_list(text_box))
    save_word_list_button.grid(row=1, column=1, padx=5)

    load_word_list_button = tk.Button(button_frame, text="Load Word List", font=("Arial", 14), command=lambda: load_word_list(text_box))
    load_word_list_button.grid(row=1, column=2, padx=5)

    # Button to enable processing and start the processing of the selected Word document, file, or folder
    start_processing_button = tk.Button(app, text="Start Processing", font=("Arial", 14), command=perform_processing)
    start_processing_button.pack(pady=10)

    # Button to select the output folder for processed files
    select_output_folder_button = tk.Button(app, text="Select Output Folder", font=("Arial", 14), command=select_output_folder)
...
# Function to select the output folder for processed files
...
# Function to perform processing (document, file, or folder) based on user selection
def perform_processing():
    # Check if any processing is enabled
    if not processing_enabled:
        messagebox.showwarning("Warning", "Processing is disabled. Please update the word list and enable processing.")
        return

    # Ask the user what type of processing they want to perform
    processing_type = messagebox.askquestion("Processing Type", "What would you like to process?\n\n"
                                                           "1. Word Document\n"
                                                           "2. File\n"
                                                           "3. Folder\n\n"
                                                           "Select 'Cancel' to exit without processing.")

    if processing_type == "yes":
        # User selected Word Document processing
        doc_path = filedialog.askopenfilename(initialdir=initial_working_directory)
        if doc_path:
            process_document(doc_path)
    elif processing_type == "no":
        # User selected File processing
        file_path = filedialog.askopenfilename(initialdir=initial_working_directory)
        if file_path:
            process_file(file_path)
    else:
        # User selected Folder processing
        folder_path = filedialog.askdirectory(initialdir=initial_working_directory)
        if folder_path:
            process_folder(folder_path)
