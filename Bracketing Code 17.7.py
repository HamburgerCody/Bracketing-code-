...
def start_gui():
    ...
    # Buttons to select a Word document or folder
    ...
    save_word_list_button.pack()

    load_word_list_button = tk.Button(app, text="Load Word List", font=("Arial", 14), command=lambda: load_word_list(text_box))
    load_word_list_button.pack()

    # Button to enable processing and start the processing of the selected Word document or folder
...
def select_output_folder():
    ...
    folder_path = filedialog.askdirectory(initialdir=output_folder)  # Use output_folder as the initial directory
    if folder_path:
...
def extract_text_from_docx(docx_filename):
    try:
        ...
        print(f"Error: Unable to read the Word document. Error: {str(e)}")
        return ""
...
def process_word_document(input_filename, word_list, result_filename, num_iterations=3):
    try:
        ...
        print(f"Error: An error occurred while processing the document. Error: {str(e)}")
        return False
    ...
    try:
        ...
        if processing_enabled:  # Check if processing is enabled
            ...
            if process_word_document(doc_path, saved_word_list, output_filename):
                print(f"Processing completed successfully. Result saved as '{output_filename}'")
            else:
...
def process_file(file_path):
    try:
        global processing_enabled  # Check if processing is enabled
        ...
        print(f"Error: An error occurred while processing the file. Error: {str(e)}")
...
def update_word_list(text_box):
    ...
    if saved_word_list:
        ...
        processing_enabled = False
        print("Word list is empty. Processing is disabled.")
...
def duplicate_folder(src_folder, dest_folder):
    try:
        ...
        print(f"Error: An error occurred while duplicating the folder. Error: {str(e)}")
...
def process_folder(folder_path):
    try:
        ...
        print(f"Processing completed successfully. Processed files saved in '{processed_folder}'")

    except Exception as e:
        print(f"Error: An error occurred while processing the folder. Error: {str(e)}")
...
def process_markdown_file(md_file, word_list):
    try:
        ...
        print(f"Error: An error occurred while processing the Markdown file. Error: {str(e)}")
