...
def process_selected():
    ...
    try:
        selected_item = filedialog.askopenfilename(initialdir=initial_working_directory)
    except Exception as e:
        logging.error(f"Error: An error occurred while processing the selected item. Error: {str(e)}")

# Function to perform the processing of the selected item (document or folder)
def perform_processing():
    global selected_item, processing_enabled  # Access the global variables
    if processing_enabled:  # Check if processing is enabled
...
def process_markdown_file(md_file, word_list):
    try:
        ...
        output_folder_path = os.path.join("Processed_Documents", os.path.splitext(sanitized_relative_path)[0])
...
def start_gui():
    ...
    start_processing_button = tk.Button(app, text="Start", font=("Arial", 14), command=toggle_processing)
...
# Function to toggle processing enabled or disabled
