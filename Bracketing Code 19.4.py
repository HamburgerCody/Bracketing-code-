...
def process_word_document(input_filename, word_list, result_folder, num_iterations=3):
    try:
        ...
        output_filename = os.path.splitext(os.path.basename(input_filename))[0]  # Original file name without extension
        full_output_path = os.path.join(result_folder, output_filename + ".docx")
...
def process_document(doc_path):
    try:
        ...
        if processing_enabled:  # Check if processing is enabled
            # Create a sanitized output file name by replacing problematic characters
            output_filename = os.path.splitext(os.path.basename(doc_path))[0]  # Original file name without extension
            output_filename = os.path.join(output_documents_folder, output_filename + ".docx")
...
def process_file(file_path):
    try:
        ...
        if processing_enabled:  # Check if processing is enabled
            ...
            # Create a sanitized output file name by replacing problematic characters
            output_filename = os.path.splitext(os.path.basename(file_path))[0]  # Original file name without extension
